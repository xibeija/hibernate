 ...
 * SessionFactory sf = new Configuration().configure().buildSessionFactory();
 * Session s = sf.openSession(); s.beginTransaction(); Category c = (Category)
 * s.get(Category.class, 3); s.delete(c); s.getTransaction().commit();
 * s.close(); sf.close(); } }
 */
 ...
 * 
 * Session s = sf.openSession(); s.beginTransaction(); Category c = (Category)
 * s.get(Category.class, 1);
 * 
 * System.out.println("log1"); System.out.println(c.getProducts());
 ...
 * 
 * Product p = (Product)s.load(Product.class, 1); System.out.println("log1");
 * System.out.println(p.getName()); System.out.println("log2");
 * 
 * s.getTransaction().commit(); s.close(); sf.close(); } }
 */
